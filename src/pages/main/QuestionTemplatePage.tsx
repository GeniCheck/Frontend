...
const INITIAL_QUESTIONS: TemplateQuestion[] = [
  { id: "q-1", title: "팀 내 협업 기여도를 1~10점으로 평가해 주세요.", type: "점수형" },
  { id: "q-2", title: "데이터 기반 의사결정 능력을 1~10점으로 평가해 주세요.", type: "점수형" },
  { id: "q-3", title: "재직 중 가장 대표적인 성과를 구체적으로 기술해 주세요.", type: "서술형" },
  {
    id: "q-4",
    title: "본인의 주요 강점 유형을 선택해 주세요.",
    type: "선택형",
    options: ["실행력 / 추진력", "분석력 / 논리력", "창의력 / 기획력", "소통력 / 협업력"],
  },
];
...
const QuestionTemplatePage: React.FC = () => {
  const [activeTab, setActiveTab] = useState<"추천 질문" | "저장된 질문" | "직접 작성">(
    "추천 질문",
  );

  const [questions, setQuestions] = useState<TemplateQuestion[]>(INITIAL_QUESTIONS);
  ...
  const buildDraftQuestion = (): TemplateQuestion => ({
    ...
    type: draftType,
    options:
      draftType === "선택형" ? ["옵션 1", "옵션 2", "옵션 3"] : undefined,
  });

  const handleAddQuestion = (question: TemplateQuestion) => {
    setQuestions((prev) => [...prev, { ...question, id: `${question.id}-${prev.length}` }]);
  };
      ...
      <div className="mx-auto w-full max-w-300 flex-1 space-y-6 p-6">
        ...
        <div className="grid grid-cols-1 items-start gap-6 lg:grid-cols-3">
              ...
              <div className="space-y-2.5">
                {RECOMMENDED_QUESTIONS.map((q) => (
                  ...
                  >
                    ...
                    <span className="text-2xs shrink-0 rounded bg-gray-100 px-2 py-0.5 font-bold text-gray-400">
                      {q.type}
                    </span>
                  </button>
                ))}
          ...
          <div className="space-y-4 lg:col-span-2">
            <div className="rounded-3xl border border-gray-200 bg-white p-6 text-left shadow-sm">
              <h3 className="text-text1 mb-4 text-sm font-black">폼 기본 정보</h3>
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-start gap-3">
                      <span className="text-text1 flex h-6 w-6 shrink-0 items-center justify-center rounded-lg bg-gray-100 text-2xs font-black">
                        {i + 1}
                      </span>
                      <span className="text-text1 text-xs font-bold">
                        {q.title}
                    ...
                    {q.type === "점수형" && (
                      <div className="flex flex-wrap gap-2">
                        {Array.from({ length: 10 }, (_, n) => n + 1).map((n) => (
                          <span
                            key={n}
                            className="text-text2 flex h-8 w-8 items-center justify-center rounded-full border border-gray-200 text-2xs font-bold"
                          >
                            {n}
                          </span>
                        ))}
